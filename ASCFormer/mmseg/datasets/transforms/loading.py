...
''' Customized'''
@TRANSFORMS.register_module()
class LoadDCTFromJPEGIO(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename = results['img_path']
        try:
            DCT_coef, qtables = self._get_jpeg_info(filename)
        ...
        if self.clip_band is None:
            results['dct'] = DCT_coef[0]
        else:
            dct = DCT_coef[0]
            results['dct'] = np.clip(dct, 0, self.clip_band)
        results['qtable'] = qtables[0]
